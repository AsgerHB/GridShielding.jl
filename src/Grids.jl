...
function Grid(granularity, lower_bounds, upper_bounds)
    ...
    Grid(granularity, dimensions, lower_bounds, upper_bounds, size, array)
end

Base.show(io::IO, grid::Grid) = println(io, 
"Grid($(grid.granularity), $(grid.lower_bounds), $(grid.upper_bounds))")
...
function box(grid::Grid, state)
	...
	for dim in 1:grid.dimensions
		...
		indices[dim] = floor(Int, (state[dim] - grid.lower_bounds[dim])/grid.granularity) + 1
	end
...
function draw(grid::Grid, slice;
	...
	if show_grid && length(grid.lower_bounds[x]:grid.granularity:grid.lower_bounds[x]) < 100
		
		vline!(grid.lower_bounds[x]:grid.granularity:grid.upper_bounds[x], 
				color=:gray, label=nothing)
		
		hline!(grid.lower_bounds[y]:grid.granularity:grid.upper_bounds[y], 
				color=:gray, label=nothing)
	end
...
function robust_grid_deserialization(file)
	...
	grid = Grid(f.granularity, f.lower_bounds, f.upper_bounds)
	for partition in grid
