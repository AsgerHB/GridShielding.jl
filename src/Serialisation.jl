...
function get_header(grid::Grid, description)	
	arrayify(x) = "{ $(join(x, ", ")) }"
	granularity = arrayify(grid.granularity)
	size = arrayify(grid.size)
	lower_bounds = arrayify(grid.bounds.lower)
...
shield_c_path = abspath("../misc/shield.c")
...
function get_libshield(shield; destination=nothing, working_dir=mktempdir())
	# Dump binary data using `ld` command
	shield_dump_o = export_to_shield_dump_o(shield; working_dir)

	# Header containing the constants needed to read the shield
	shield_h = working_dir ⨝ "shield.h"
	write(shield_h, get_header(shield, "foo"))
...
shield_c_path = "/home/asger/.julia/dev/GridShielding/misc/shield.c"


"""
    get_libshield(shield::Grid; [destination, working_dir])
...
- `destination`: Filepath. The compiled file will be copied to here.
- `working_dir`: Temp folder by default. Several files will be generated here.
"""
function get_libshield(shield::Grid; destination=nothing, working_dir=mktempdir())
	# Dump binary data using `ld` command
	shield_dump_o = export_to_shield_dump_o(shield; working_dir)
